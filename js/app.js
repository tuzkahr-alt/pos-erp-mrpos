...
document.addEventListener('DOMContentLoaded', () => {
    // Inicializar módulo de Letras (si existe)
    ...
    const renderPOSProducts = (customList = null) => {
        ...
        displayList.forEach(p => {
            ...
            div.innerHTML = `
                ...
                <div class="stock ${p.stock <= 5 ? 'text-red' : ''}">Uds: ${p.stock}</div>
            `;
    ...
    globalSearch.addEventListener('input', (e) => {
        ...
        matchingSales.slice(0, 5).forEach(s => {
            ...
            div.innerHTML = `<span><strong>Folio: ${s.id}</strong> | ${s.method}</span> <span>${formatMoney(s.total)}</span>`;
            div.onclick = () => { alert(`Venta Folio ${s.id}\nComprador: ${s.clientId}\nTotal: ${formatMoney(s.total)}`); globalResults.classList.add('hidden'); };
    ...
    // ===== 5. COBRANZAS (LETRA A, B, C) =====
    const loadCredits = () => {
        const body = document.getElementById('credits-body');
        body.innerHTML = '';
        db.data.quotas.forEach(q => {
            const tr = document.createElement('tr');
            // Transformar numero a letra: 1 -> A, 2 -> B...
            const letraCorrelativa = getLetter(q.num_quota);
            tr.innerHTML = `
                <td><strong>${q.clientName}</strong></td>
                <td>${q.saleId}</td>
                <td class="text-blue fw-bold">Letra ${letraCorrelativa}</td>
                <td>${q.dueDate}</td>
                <td>${formatMoney(q.amount)}</td>
                <td>$0</td>
                <td><span class="badge ${q.status === 'pagado'?'badge-green':'badge-orange'}">${q.status.toUpperCase()}</span></td>
                <td><button class="btn-primary" onclick="payQuota('${q.id}')">Pagar</button></td>
            `;
            body.appendChild(tr);
        });
    };

    window.payId = (id) => { /* Dummy for letters module */ };
    ...
    const loadReportsCharts = () => {
        ...
        chartD = new Chart(ctxD, {
            type: 'doughnut',
            data: { labels: ['Ventas','Créditos'], datasets: [{ data: [350000, 120000], backgroundColor: ['#2563eb', '#38c172'] }] },
            options: { responsive: true, maintainAspectRatio: false }
        });

        chartW = new Chart(ctxW, {
            type: 'line',
            data: { labels: ['Lun','Mar','Mie','Jue','Vie','Sab','Dom'], datasets: [{ label: 'Ventas Semanales', data:[12, 19, 3, 5, 2, 3, 7], borderColor:'#1a73e8' }] },
            options: { responsive: true, maintainAspectRatio: false }
        });
    };

    // ===== 7. INVENTORY & MAESTRO PRODUCTOS =====
    const loadInventory = () => {
        ...
        db.data.products.forEach(p => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${p.sku}</td><td>${p.name}</td><td>${p.talla || 'U'} / ${p.genero || 'U'}</td>
    ...
    document.getElementById('btn-save-product').addEventListener('click', () => {
        const p = {
            ...
            talla: document.getElementById('new-prod-talla').value,
            genero: document.getElementById('new-prod-genero').value,
            ...
            provider: document.getElementById('new-prod-prov').value
        };
        db.addProduct(p);
